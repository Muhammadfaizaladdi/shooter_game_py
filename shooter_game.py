...
finish = False
#main game loop:
run = True #the flag is reset by the window close 
life = 3
num_fire = 0
reload_time = False
while run:
 ...
 #the game itself: actions of sprites, checking the rules of the game, redrawing
   if not finish:
       ...
       if reload_time:
            cur_reload_time = t()
            # jika waktu yang sudah lewat sejak reload_time < 3 detik
            if cur_reload_time - start_reload <= 3:
                # tampilkan teks reload time
                reload = font2.render("Reload the Bullet", 1, (150,0,0))
                window.blit(reload, (260,400))
            # kalau sudah lebih dari 3 detik:
            else:
                num_fire = 0
                reload_time = False
                # maka jumlah peluru dan reload time kembali ke  
